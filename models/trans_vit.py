...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attn=False, value_norm=False):
        B, N, C = x.shape
        ...
        if return_attn:
            attn_map=attn[:,:,0,:]
            if value_norm:
                attn_map=(attn_map * v.norm(dim=-1))
                attn_map=F.normalize(attn_map, dim=-1, p=1)
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attn=False, value_norm=False):
        if return_attn:
            res, attn_map = self.attn(self.norm1(x), return_attn=return_attn,value_norm=value_norm)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if proj_signal:
            ...
            self.proj = mlp.get_mlp(hidden_dims, self.teacher_embed_dim, proj_layers) 
        # else:
        #     self.teacher_embed_dim=embed_dim
        ### else:
        ###     self.proj=None
        ########### ATTENCION: proj
        ...
        # Classifier head
        if proj_signal:
            self.head = nn.Linear(teacher_embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        else:
