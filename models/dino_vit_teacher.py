    ...
    def get_last_selfattention(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if i < len(self.blocks) - 1:
                ...
                return blk(x, return_attn=True)[1]
...
def vit_tiny_patch32_depth8(patch_size=32, **kwargs):
    model = VisionTransformer(
        patch_size=32, embed_dim=192, depth=8, num_heads=3, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
def vit_tiny_patch8(patch_size=8, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
def vit_small_patch8(patch_size=8, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
#### ======= vit large ================
def vit_large_patch16(patch_size=16, **kwargs):
    model = VisionTransformer(
